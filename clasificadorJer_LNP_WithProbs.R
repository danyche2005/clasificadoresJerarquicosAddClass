...
funTrainingLNP3 <- function(arbol, dataTraining, nivel="raiz", tecnicaEntrenamiento,probabilitiesOp=FALSE,balanceOpt=FALSE,adaptarModelo=FALSE){
  ...
  if(hijos>1){
    ...
    for(i in 1:hijos){
      ...
      if(!nodoHijo$isLeaf){
        ...
        if(condicionAD){
          funTrainingLNP3(arbol = nodoHijo,dataTraining = hijoDataTraining,nivel = nodosDescendientes[1],tecnicaEntrenamiento =  tecnicaEntrenamiento,probabilitiesOp = probabilitiesOp,balanceOpt = balanceOpt,adaptarModelo = adaptarModelo  )
        }
    ...
    #plot(as.dendrogram(arbol))
    #plot(as.iarbol(arbol, directed = TRUE, direction = "climb"))
    ...
    if(balanceOpt){     # if(balanceOpt==TRUE && arbol$name == "cNew"){
    ...
    instancias<-(paste(as.numeric(table(tempTraining$claseAPredecir)), collapse="#"))
...
#Utilizado para Procesar Textos
potenciarModelo <- function(arbol, dataTraining, nivel="raiz", tecnicaEntrenamiento,probabilitiesOp=FALSE){
  ...
  if(hijos>1){
    ...
    for(i in 1:hijos){
      ...
      if(!nodoHijo$isLeaf){
        ...
        potenciarModelo(arbol = nodoHijo,dataTraining = hijoDataTraining,nivel = nodosDescendientes[1],tecnicaEntrenamiento =  tecnicaEntrenamiento,probabilitiesOp = probabilitiesOp)
    ...
    tempTraining$claseAPredecir<-factor(tempTraining$claseAPredecir)
      ...
      salidasPosibles<-(paste(levels(tempTraining$claseAPredecir), collapse="#"))
      #Tambien guardo el clasificador en el grafico
      arbol$clasificadorAdapt<-clasificadorNodo
      arbol$entradaAdapt<-nivel
      arbol$salidasAdapt<-salidasPosibles
      arbol$equivalenciaAdapt<-''
      arbol$cantEntrenamientoAdapt<-(paste(paste(names(table(tempTraining$claseAPredecir)),sep = " ")," :",as.character(table(tempTraining$claseAPredecir)), collapse=" | "))
    #}
  }
  return(arbol)
}
