...
####The next function creates the new node in the classifier tree using the training and validation sets and applying SMOTE
 funTrainingLNP2 <- function(graph, dataTraining, nivel="raiz", tecnicaEntrenamiento,probabilitiesOp=FALSE){
  ...
  if(hijos>1){
    ...
    for(i in 1:hijos){
      ...
      if(!nodoHijo$isLeaf){
        ...
        funTrainingLNP2 (graph = nodoHijo, dataTraining = hijoDataTraining, nivel = nodosDescendientes[1], tecnicaEntrenamiento = tecnicaEntrenamiento, probabilitiesOp = probabilitiesOp)
      }else{
...
topDownTesting2 <- function(datoTest,graph,metodo){
  ...
  clasePredicha<-as.character( prediceLibTecnicas( modelo=graph$clasificadorBalanced, datosTest=datoTest, metodo, obtenerProb=FALSE ) ) 
